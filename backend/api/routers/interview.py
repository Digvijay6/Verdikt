...
def redeem(body: RedeemRequest) -> RedeemResponse:
    ...
    package = InterviewPackage(
        ...
        questions=[],  # filled from the job's rubric (Lane 1 generates this)
        resume_summary=application.get("parsed_resume", {}).get("summary", "")
            if application.get("parsed_resume") else "",
        rubric_version="v1",
        language="en",
    )
