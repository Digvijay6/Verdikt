...
| The questions asked are attributable | `questions_model_id`, `questions_prompt_version`, `questions_rubric_version` on every application. Since D35 each candidate is asked different questions, so "which questions, generated by what, scored against which anchors" is now part of the decision record rather than a property of the job |
...
- [ ] Bias audit plan — **the unit of comparison is the competency, not the
      question** (D35). Auditing "how did group A and group B score on question
