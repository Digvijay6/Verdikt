...
2. **`total_years_experience` is computed, not copied.** Sum the employment
   periods, counting overlaps once. Ignore any self-reported "10+ years" claim
   in a summary line — that is a claim, not evidence. If dates are too vague to
   compute, return `null` rather than guessing. Downstream treats `null` as
   "unknown" and passes the candidate on; a wrong number would silently reject
   them.
3. A role with no end date is current: `end` is `null`.
4. `skills` is for technologies, tools, languages, and methods. Not soft skills,
   not job titles, not company names.
5. Preserve the candidate's own wording for titles and company names. Do not
   normalize "Sr. SWE II" into something tidier.
6. Dates: use the first day of the month when only a month and year are given,
